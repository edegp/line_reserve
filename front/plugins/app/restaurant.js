    ...
    return {
        ...
        async getAreaShops() {
            ...
            for (const record of data) {
                ...
                for (const shopRecord of record.shop) {
                    ...
                    shop.holiday = app.$restaurant.utils.convertWeekdays(
                        shopRecord.closeDay
                    );
                    ...
                    shop.smoking =
                        shopRecord.smokingFlg == 1
                            ? _i18n.restaurant.allowed
                            : _i18n.restaurant.not_allowed;
                    shop.map = shopRecord.coordinate
                        ? shopRecord.coordinate
                        : null;
        ...
        async getCourses(shopId) {
            ...
            for (const record of data) {
                ...
                course.text = `${course.name} (${_i18n.restaurant.yen.replace(
                    "{price}",
        ...
        async getMonthlyReservationStatus(shopId, month, restaurant) {
            ...
            for (const dayRecord of calendar) {
                ...
                const workingStatus = app.$restaurant.utils.isHoliday(
                    day,
                    restaurant.holiday
                );
        ...
        async deleteReserve(token, names, day, start) {
            // LIFF ID Token取得
            const idToken = store.state.lineUser.idToken;
            ...
            // 予約登録
            const data = await this[_module].deleteReserve(params);
            if (!data) {
                return null;
            }
        ...
        utils: {
            ...
            monthList(count) {
                ...
                if (_i18n.type == "en") {
                    mm = app.$utils.englishMonth(mm);
                }
                months.push({
                    text: _i18n.restaurant.yyyymm
                        .replace("{year}", yyyy)
                        .replace("{month}", mm),
            ...
            createEvents(day, restaurant) {
                ...
                for (let dt = ftime; dt < ttime; dt = dt + 1000 * 60 * 30) {
                    ...
                    const stime = `${day} ${("00" + startDate.getHours()).slice(
                        -2
                    )}:${("00" + startDate.getMinutes()).slice(-2)}`;
                    const etime = `${day} ${("00" + endDate.getHours()).slice(
                        -2
        ...
        axios: {
            ...
            shopStatusCalendar: async (shopId, month) => {
                ...
                const params = {
                    locale: store.state.locale,
                    shopId: shopId,
                    preferredYearMonth: `${month.substr(0, 4)}-${month.substr(
                        4,
                        2
                    )}`,
                };
        ...
        amplify: {
            ...
            areaData: async () => {
                ...
                // GET送信
                try {
                    response = await app.$amplify.API.get(
                        "LambdaAPIGateway",
            ...
            shopStatusCalendar: async (shopId, month) => {
                ...
                const myInit = {
                    queryStringParameters: {
                        locale: store.state.locale,
                        shopId: shopId,
                        preferredYearMonth: `${month.substr(
                            0,
                            4
                        )}-${month.substr(4, 2)}`,
                    },
            ...
            deleteReserve: async (params) => {
                ...
                // POST送信
                try {
                    response = await app.$amplify.API.delete(
                        "LambdaAPIGateway",
                        `${_stage}/reservation_delete`,
                        myInit
                    );
            ...
            reserve: async (params) => {
                ...
                // POST送信
                try {
                    response = await app.$amplify.API.post(
                        "LambdaAPIGateway",
                        `${_stage}/reservation_put`,
                        myInit
                    );
